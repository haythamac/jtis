...
// 	$exam=new manage_exam_class;;
// 	$ques=htmlentities($question_identification);
// 	$answer=htmlentities($answer_identification);

// 	$query="insert into question_exam_identification values('','$ques','$answer_identification','$cat', '$diff')"; //$cat value comes from select name attribute

// 	// mysqli_query($con,$query);
// 	if ($exam->add_exam($query)) 
// 	{
// 		header("location:manage_exam.php?run=success");
// 	}
//  }
...
{	
	...
	$query="insert into question_exam_identification values('','$question_identification','$answer_identification','$cat', '$diff_identification')"; //$cat value comes from select name attribute
	$add_to_difficulty = "INSERT INTO difficulty VALUES ('', '$diff_identification', '$cat', '$zero', '$test_type')";
	// mysqli_query($con,$query);
	if ($exam->add_exam_identification($query)) 
	{	$exam->add_exam_identification($add_to_difficulty);
		header("location:manage_exam.php?run=success");
	}
...
{	
	...
	$query="insert into question_exam values('','$ques','$option1','$option2','$option3','$option4','$matchedanswer','$cat', '$diff')"; //$cat value comes from select name attribute
...
//================================================================================================================
 		...
 		// add a new question code from manage_exam.php  from tab 3 (add questions) 

 
// extract($_POST);

// $selected_difficulty=htmlentities($exampleFormControlSelect1);
