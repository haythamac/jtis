...
class quizUsers
{
	...
	public function showProfileQuizScore(){

		$current_user_id = $_SESSION['user']['user_id'];
		// get score quiz details in database
		$query=$this->conn->query("SELECT * FROM score_quiz WHERE student_id = $current_user_id");
		while($row=$query->fetch_array(MYSQLI_ASSOC)){
			$this->quiz_score_data[] = $row;
		}
		return $this->quiz_score_data;
	}

	public function showActivityLog(){
		$current_user_id = $_SESSION['user']['user_id'];
		// get activity log in database
		$query=$this->conn->query("SELECT * FROM activity_log WHERE user_id = $current_user_id");
		while($row=$query->fetch_array(MYSQLI_ASSOC)){
	...
	public function showActivityLogAdmin(){
		// get activity log in database
	...
	public function showProfileQuizScoreAdmin(){
		// get score quiz details in database
		$query=$this->conn->query("SELECT * FROM score_quiz");
		while($row=$query->fetch_array(MYSQLI_ASSOC)){
			$this->quiz_score_data[] = $row;
		}
		return $this->quiz_score_data;
	...
	public function show_taken_quiz()
	{
		$query = $this->conn->query("SELECT * FROM quiz_taken WHERE user_id='" . $_SESSION["user"]["user_id"] . "'");  
	...
	{
		$query=$this->conn->query("select * from question_quiz where course_id='$course_id' AND difficulty='$diff_type' ORDER BY RAND()");
	...
	{
		$query=$this->conn->query("select * from question_quiz_identification where course_id='$course_id' AND difficulty='$diff_type' ORDER BY RAND()");
...

	{
		...
		if($diff_type != 'hard'){
			$query=$this->conn->query("select id,answer from question_quiz where course_id='$course_id' AND difficulty='$diff_type'");
			while($this->rand_row=$query->fetch_array(MYSQLI_ASSOC))      // while loop to fetch all data one by one and store in cat_data array variable
			{	
				$answer_id = $this->rand_row['id'];
				$answer = "SELECT answer FROM question_quiz WHERE id='$answer_id'";
			...
			$query=$this->conn->query("select id,answer from question_quiz_identification where course_id='$course_id' AND difficulty='$diff_type'");

			while($this->rand_row=$query->fetch_array(MYSQLI_ASSOC))      // while loop to fetch all data one by one and store in cat_data array variable
			{	
				$answer_id = $this->rand_row['id'];
				$identification_answer = "SELECT answer FROM question_quiz_identification WHERE id='$answer_id'";
