...
class users
{
	...
	{
		$query=$this->conn->query("select * from question_test where course_id='$course_id' AND difficulty='$diff_type' ORDER BY RAND()");
...

	{
		...
		$query=$this->conn->query("select id,answer from question_test where course_id='$course_id' AND difficulty='$diff_type'");
		...
		{	
			...
			elseif ($this->rand_row['answer']==$_POST[$this->rand_row['id']]){    //if answer is match 
				$right++;
			}
			else
			{
