            ...
            <div><!--- class="collapse" id="myid"--->
              <div class="form-group">
                ...
                /* print_r($taken_id);
                  ...
                  { 
                    foreach ($profile->quiz_taken as $lock => $col)
                    {
                      print_r($col);
                    }
                ...
                $quiz_taken_assoc = $profile->quiz_taken;
                $taken_difficulty = array();
                foreach ($quiz_taken_assoc as $index) {
                ...
                <form method="POST" action="question_show.php">
                  ...
                  <button type="submit" id="start_quiz" name="start_quiz" class="btn btn-success mt-3" disabled>START</button>
                  <br>
                  <label style="color: red; font-size:16px; margin-top:1rem;" id="quiz_status"></label>
        ...
        $(document).ready(function () {
          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              console.log('Course name: ',data.language_name);
              $('#topic').html('');
          ...
          // Function to check if the quiz is taken
          var difficulty = $('#difficulty option:selected').val();
          function isQuizTaken(topic_id, difficulty, callback) {
            $.ajax({
              type: 'POST',
              url: 'check_quiz_taken.php',
              data: {
                topic_id: topic_id,
              ...
              error: function (xhr, status, error) {
                ...
                callback(false); // Assume the quiz is not taken in case of an error
              }
            });
          }
          ...
          $('#difficulty').change(function () {
            ...
            var cat_id = $('#topic option:selected').val();
            $.ajax({
              type: "POST",
              url: "save_variables.php",
              data: {
              ...
              success: function () {
                ...
                console.log('Course id: ', cat_id);
                // Call the function to check if the quiz is taken
                isQuizTaken($('#topic option:selected').val(), difficulty, function (isTaken) {
                  if (isTaken === 'yes') {
                    // quiz is taken, perform the desired action (e.g., show a message)
                    console.log('quiz is already taken');
                    $('#start_quiz').prop('disabled', true);
                    $('#quiz_status').text('This quiz has already been taken.');
                  } else {
                    // quiz is not taken, continue with your logic
                    console.log('quiz is not taken');
                    $('#start_quiz').prop('disabled', false);
                    $('#quiz_status').text(''); // Clear the quiz status message
                  }
                });
              }
            });
